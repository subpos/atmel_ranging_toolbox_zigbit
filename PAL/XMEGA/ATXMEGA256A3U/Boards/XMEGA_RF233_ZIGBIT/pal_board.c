...
#if (BOARD_TYPE == XMEGA_RF233_ZIGBIT)
...
void event_system_init(void)
{
    ...
    EVSYS_CH2MUX = EVSYS_CHMUX_PRESCALER_32_gc;
#elif (F_CPU == (16000000UL))
    /* Route system clock (16MHz) with prescaler 16 through event channel 0 */
    EVSYS_CH0MUX = EVSYS_CHMUX_PRESCALER_16_gc;
	EVSYS_CH2MUX = EVSYS_CHMUX_PRESCALER_16_gc;
    
#elif (F_CPU == (8000000UL))
    /* Route system clock (8MHz) with prescaler 8 through event channel 0 */
    EVSYS_CH0MUX = EVSYS_CHMUX_PRESCALER_8_gc;
	EVSYS_CH2MUX = EVSYS_CHMUX_PRESCALER_8_gc;

#elif (F_CPU == (4000000UL))
    /* Route system clock (4MHz) with prescaler 4 through event channel 0 */
    EVSYS_CH0MUX = EVSYS_CHMUX_PRESCALER_4_gc;
	EVSYS_CH2MUX = EVSYS_CHMUX_PRESCALER_4_gc;

#else
#   error Unknown system clock
#endif

    /* Route port C pin 1 (DIG2) through event channel 1; same on zigbit */
    EVSYS_CH1MUX = EVSYS_CHMUX_PORTC_PIN1_gc;

    /* Route port R pin 1 (
	 from transceiver) through event channel 2 */
    //Instead the code has been changed to always use the system clock.
    //If using the external OSC, it just uses event timer 0
#ifndef EXTERNAL_OSC
	EVSYS_CH2MUX = EVSYS_CH0MUX; //If running internal OSC, just use the same event timer
#else
    //EVSYS_CHMUX_PRESCALER_16_gc;
	//EVSYS_CH2MUX = EVSYS_CHMUX_PORTF_PIN7_gc;
	
    //Set event channel 7 out to DIG2
	//Output event channel 1/7 to Pin (D7)
    //FREQ_OUT_PORT.DIRSET = _BV(FREQ_OUT_PIN);
	
	//Select Event Channel 7
	//PORTCFG.EVOUTSEL = PORTCFG_EVOUTSEL_2_gc;
...
void clock_init(void)
{
    ...
    OSC.DFLLCTRL &= ~ ((OSC_RC32MCREF0_bm) | (OSC_RC32MCREF1_bm));
...
void timer_init_non_generic(void)
{
    ...
    //Use event channel 1 as clock source
